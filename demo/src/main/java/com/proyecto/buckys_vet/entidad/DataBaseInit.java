...
public class DataBaseInit implements ApplicationRunner {
    ...
    private String generarCorreo(String nombre, String apellido, int i) {
        return nombre.toLowerCase().replace(" ", "") + "." + apellido.toLowerCase().replace(" ", "") + i + "@example.com";
    }
    ...
    public void run(ApplicationArguments args) throws Exception {
        ...
        for (int i = 0; i < 50; i++) {
            ...
            String nombre = esHombre ? NOMBRES_HOMBRES[random.nextInt(NOMBRES_HOMBRES.length)] : NOMBRES_MUJERES[random.nextInt(NOMBRES_MUJERES.length)];
            String apellido = APELLIDOS[random.nextInt(APELLIDOS.length)];
            String email = generarCorreo(nombre, apellido, i);
            String imagenUrl = esHombre ? IMAGENES_DUENOS_HOMBRES[random.nextInt(IMAGENES_DUENOS_HOMBRES.length)] : IMAGENES_DUENOS_MUJERES[random.nextInt(IMAGENES_DUENOS_MUJERES.length)];

            Dueno dueno = new Dueno();
            dueno.setCedula(1000000L + i);
            dueno.setNombre(nombre + " " + apellido);
            dueno.setEmail(email);
            dueno.setTelefono("300000000" + i);
            dueno.setImagenUrl(imagenUrl);
            dueno.setPassword(generarPasswordDeterminista(nombre, i));
        ...
        for (int i = 0; i < 100; i++) {
            ...
            String imagenMascota = especie.equals("Perro") ? IMAGENES_PERROS[random.nextInt(IMAGENES_PERROS.length)] : IMAGENES_GATOS[random.nextInt(IMAGENES_GATOS.length)];
        ...
        for (int i = 0; i < 10; i++) {
            ...
            String imagenUrl = esMujer 
                ? IMAGENES_VETERINARIOS_MUJERES[random.nextInt(IMAGENES_VETERINARIOS_MUJERES.length)]
                : IMAGENES_VETERINARIOS_HOMBRES[random.nextInt(IMAGENES_VETERINARIOS_HOMBRES.length)];
        ...
        for (int i = 0; i < 10; i++) {
            Tratamiento t = new Tratamiento(java.time.LocalDate.now().minusDays(i), "Tratamiento " + i, vets.get(i), mascotas.get(i), meds.get(i), i + 1);
            tratamientos.add(t);
        }
